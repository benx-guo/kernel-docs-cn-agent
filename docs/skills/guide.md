...
选定文件类型后，进入文件浏览。交互流程与 `/work` 完全一致（参见 `docs/skills/work.md` "无参数时的文件选择流程"）。
